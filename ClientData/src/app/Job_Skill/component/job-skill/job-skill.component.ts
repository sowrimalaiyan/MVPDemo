...
@Component({
  ...
  styleUrls: ['./job-skill.component.css']
})
export class JobSkillComponent implements OnInit {
  ...
  constructor(private jobService: JobService, private skillService: SkillService) { }
      ...
      next: (response) => {
        ...
        if (response) {
          ...
          for (let i = 0; i < response.length; i++) {
            delete response[i].jobSkillId;
            var tempData= this.dropdownData;
            selectedSkills.push(tempData.find((item:any) => item.id === response[i].id));
          }
  ...
  onAddSubmit() {
    ...
    this.jobService.add(params).subscribe({
      next: (response) => {
  ...
  onEditSubmit() {
    ...
    this.jobService.edit(params).subscribe({
      next: (response) => {
  ...
  delete(data: any) {
    ...
    this.jobService.delete(params).subscribe({
      next: (response) => {
